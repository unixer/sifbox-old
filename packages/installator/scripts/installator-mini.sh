...
menuentry "Start $DISTRO $ARCH $VERSION" {
  ...
  linux /boot/vmlinuz root=UUID=${ID_FS_UUID} rootfstype=${target_fs} rootfs=flat quiet vga=789 loglevel=3 rw
  initrd /boot/initrd
}
...
LABEL sifbox
  ...
  MENU DEFAULT
  KERNEL /boot/vmlinuz
  APPEND initrd=/boot/initrd root=UUID=${ID_FS_UUID} rootfs=flat quiet vga=789 loglevel=3 rw
